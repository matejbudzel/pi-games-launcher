...
cvt=$(setting framebuffer_hdmi_cvt)
boot=${BOOT_CONFIG:-/boot/config.txt}
[ -z "${BOOT_CONFIG:-}" ] && [ -f /boot/firmware/config.txt ] && boot=/boot/firmware/config.txt
